var mapstore_utils = {
	...
	/**
	 * Clean `request` parameter on the URL
	 **/
	cleanUrl: function(server){
		var url = server;

		var getCapabilitiesParam = "request=getcapabilities";
		var startCapabilitiesIndex = url.toLowerCase().indexOf(getCapabilitiesParam);
		var endCapabilitiesIndex = startCapabilitiesIndex + getCapabilitiesParam.length;
		if(startCapabilitiesIndex > -1){
			if(url[startCapabilitiesIndex-1] == '&'){
				startCapabilitiesIndex--;
			}
			if((startCapabilitiesIndex + getCapabilitiesParam.length) < url.length){
				endCapabilitiesIndex++;
			}
			server = url.substring(0, startCapabilitiesIndex) + url.substring(endCapabilitiesIndex);
		}
